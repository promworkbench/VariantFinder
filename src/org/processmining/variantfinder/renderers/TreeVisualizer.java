...
/*
 * TreeVisualizer.java Copyright (C) 1999-2012 University of Waikato, Hamilton,
 * New Zealand
 * 
 * 
 *
...
public class TreeVisualizer extends PrintablePanel
	 ...
	 */
	public TreeVisualizer(TreeDisplayListener tdl, String dot, NodePlace p, int classIndex) {
		super();
		...
		if (e.getActionCommand() == null) {
			...
			if (m_focusNode >= 0) {
				...
				if ((inst = m_nodes[m_focusNode].m_node.getInstances()) != null) {
					...
					//this is for J48 trees
					Node node = m_nodes[m_focusNode].m_node;
					String nodeName = node.getLabel();
					
					NodeChart chart;
					if(inst.classAttribute().isNumeric())
						chart = new NumericClassChart();
					else
						chart = new CategoricalClassChart();
					
					JFrame nf = new JFrame();
					nf.setSize(400, 300);
					nf.getContentPane().add(chart.getChart(inst, nodeName));
					nf.setVisible(true);
				} else {
			...
			if (m_listener != null) {
				...
				JOptionPane.showMessageDialog(this,
						"Sorry, there is no " + "available Decision Tree to " + "perform this operation on.", "Sorry!",
						JOptionPane.WARNING_MESSAGE);
			}
		}
	}
			...
			for (int noa = 0; noa < m_numNodes; noa++) {
				if (m_nodes[noa].m_quad == 18) {
					...
					if (e.getX() <= m_nodes[noa].m_center + m_nodes[noa].m_side
							&& e.getX() >= m_nodes[noa].m_center - m_nodes[noa].m_side && e.getY() >= m_nodes[noa].m_top
